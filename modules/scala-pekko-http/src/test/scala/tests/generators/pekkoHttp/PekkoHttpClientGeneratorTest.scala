...
class PekkoHttpClientGeneratorTest extends AnyFunSuite with Matchers with SwaggerSpecRunner with ScalaMetaMatchers {
  import scala.meta._
  ...
  test("Ensure traced responses are generated") {
    val (
      ...
      _
    ) = runSwaggerSpec(scalaInterpreter)(spec)(Context.empty.copy(framework = Some("pekko-http"), tracing = true), "pekko-http")
    val cmp = companionForStaticDefns(staticDefns)
